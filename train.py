...
def train(hyp, opt, device, tb_writer=None):
    ...
    # Process 0
    if rank in [-1, 0]:
        testloader, testdata = create_dataloader(test_path, imgsz_test, batch_size * 10, gs, opt,  # testloader
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch-size', type=int, default=50, help='total batch size for all GPUs') # LLVIP 1张卡用54差不多，7张卡注意乘以7，但是最好不要用奇数张卡
    ...
    parser.add_argument('--workers', type=int, default=10, help='maximum number of dataloader workers') # 注意4096下总数8，16382
    ...
    if not opt.evolve:
        ...
        if opt.global_rank in [-1, 0]:
            ...
            with open('total_time.txt', 'a') as f:
                f.write(f'{time.time()} : Total time: {total_time}s\n')

            print(f'Total time: {total_time}s saved to total_time.txt')
