...
def train_rgb_ir(hyp, opt, device, tb_writer=None,model=None):
...
def start_train(model = None):
    ...
    parser.add_argument('--batch-size', type=int, default=55, help='total batch size for all GPUs') # 1张卡用50，剪枝后60%用60，保持这个数就行
    ...
    if not opt.evolve:
        ...
        if opt.global_rank in [-1, 0]:
            ...
            train_rgb_ir(hyp, opt, device, tb_writer,model)
